...
For the first analysis I looked at how a sinusoidal relationship here, to match up with the cyclical pattern of days. The sine and cosine terms are both significant, but the model has a very low adjusted R^2 so it has little predictive power. The following plot shpws the models fit, with the max marked in red and the min marked in blue. 

```{r}
model = lm(STATISTICAL_MURDER_FLAG ~ I(sin(pi*OCCUR_HOUR/12)) + I(cos(pi*OCCUR_HOUR/12)), data = shootings)
summary(model)
```

```{r}
x = seq(0,23,length.out = 100)
y = predict(model, data.frame(OCCUR_HOUR = x))
plot(x,y, ylim = c(0,1), type='l')
points(x[which.max(y)], max(y), col = 'red')
points(x[which.min(y)], min(y), col = 'blue')
```
